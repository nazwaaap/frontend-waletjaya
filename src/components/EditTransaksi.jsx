...
export default function EditTransaksi() {
  ...
  const calculatePreview = (modalPerGram, beratTerjual, hargaJual) => {
    ...
    setPreview({ totalModal, totalHargaJual, totalLaba, persenLaba, persenMarkup });
  };
        ...
        <div className="bg-white border-b border-gray-200 shadow-sm">
          <div className="px-4 py-3">
            <div className="flex items-center gap-3">
              <button onClick={() => navigate("/transaksi-penjualan")} className="p-2 hover:bg-gray-100 rounded-lg transition-colors">
                <ArrowLeft className="w-5 h-5 text-gray-700" />
              </button>
              <div className="flex-1 min-w-0">
                <h1 className="text-base font-bold text-navy">Edit Transaksi</h1>
                <p className="text-[10px] text-gray-500">Perbarui informasi transaksi</p>
              </div>
              <Edit3 className="w-5 h-5 text-navy hidden sm:block" />
            </div>
          </div>
        </div>
                ...
                <div className="bg-yellow-50 border border-gray-300 rounded-lg p-3 space-y-2">
                  ...
                  <div className="grid grid-cols-2 gap-2 text-xs">
                    <div className="bg-white rounded-md p-2 border border-gray-300">
                      ...
                      <p className="font-medium text-gray-900">Rp {formatCurrency(formData.hargaJualPerGram)}</p>
                    </div>
                    <div className="bg-white rounded-md p-2 border border-gray-300">
                  ...
                  <div className="pt-2 space-y-1.5 text-xs">
                    <div className="flex justify-between items-center">
                      ...
                      <span className="font-medium text-gray-900">Rp {formatCurrency(preview.totalModal)}</span>
                    </div>
                    <div className="flex justify-between items-center">
                      <span className="text-gray-700">Total Penjualan:</span>
                      <span className="font-medium text-gray-900">Rp {formatCurrency(preview.totalHargaJual)}</span>
                    </div>
                    ...
                    <div className="flex justify-between items-center bg-yellow-100 rounded px-2 py-1">
                      ...
                      <span className="font-medium text-gray-600">{preview.persenLaba.toFixed(2)}%</span>
                    </div>
                  </div>
                </div>
